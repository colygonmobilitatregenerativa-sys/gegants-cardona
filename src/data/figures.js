export const figuresData = [
  {
    id: 'batallador',
    name: 'El Batallador',
    subtitle: {
      ca: 'Gegant Guerrer del Barri de Sant Miquel',
      es: 'Gigante Guerrero del Barrio de San Miguel',
      en: 'Warrior Giant of the Sant Miquel Quarter'
    },
    category: 'sant-miquel',
    categoryLabel: {
      ca: 'Barri de Sant Miquel',
      es: 'Barrio de San Miguel',
      en: 'Sant Miquel Quarter'
    },
    year: {
      ca: '1957 (Estrena: 16 de juny de 1957)',
      es: '1957 (Estreno: 16 de junio de 1957)',
      en: '1957 (Debut: June 16, 1957)'
    },
    sculptor: {
      ca: 'El Ingenio (Barcelona) · Restauracions de Manel Casserres i Toni Mujal',
      es: 'El Ingenio (Barcelona) · Restauraciones de Manel Casserres y Toni Mujal',
      en: 'El Ingenio (Barcelona) · Restorations by Manel Casserres & Toni Mujal'
    },
    height: '3,80 m',
    weight: '60 kg',
    location: {
      ca: 'Cardona - Barri de Sant Miquel',
      es: 'Cardona - Barrio de San Miguel',
      en: 'Cardona - Sant Miquel Quarter'
    },
    image: './images/batallador.jpg',
    imagePosition: 'center 55%',
    description: {
      ca: 'El Gegant Batallador és la figura guerrera i senyorial del Barri de Sant Miquel de Cardona. Estrenat solemnement el 16 de juny de 1957 gràcies a la iniciativa veïnal i finançat per Jaume Vendrell Rosas, forma parella històrica amb la geganta l\'Esperança. Encarna la valentia militar i l\'esperit heroic dels defensors de la fortalesa del Castell de Cardona al llarg dels segles.',
      es: 'El Gigante Batallador es la figura guerrera y señorial del Barrio de San Miguel de Cardona. Estrenado solemnemente el 16 de junio de 1957 gracias a la iniciativa vecinal y financiado por Jaume Vendrell Rosas, forma pareja histórica con la giganta l\'Esperança. Encarna la valentía militar y el espíritu heroico de los defensores de la fortaleza del Castillo de Cardona a lo largo de los siglos.',
      en: 'The Batallador Giant is the noble warrior figure of Cardona’s Sant Miquel quarter. Solemnly unveiled on June 16, 1957 through local neighborhood initiative and financed by Jaume Vendrell Rosas, he pairs historically with the Esperança giantess. He embodies the martial valor and heroic spirit of the defenders of Cardona Castle throughout the centuries.'
    },
    vestimenta: {
      ca: 'Imponent capa cerimonial de vellut carmesí ricament brodada amb sanefes daurades, casc guerrer amb cimera daurada en forma de drac/ocell mitològic, túnica noble amb escut central que il·lustra el paisatge de la vall salina, i la cèlebre destral de combat medieval que empunya amb fermesa a la mà dreta.',
      es: 'Imponente capa ceremonial de terciopelo carmesí ricamente bordada con cenefas doradas, casco guerrero con cimera dorada en forma de dragón/ave mitológica, túnica noble con escudo central que ilustra el paisaje del valle salino, y la célebre hacha de combate medieval que empuña con firmeza en su mano derecha.',
      en: 'Imposing crimson velvet ceremonial cape richly embroidered with gold borders, warrior helmet crowned with a mythical dragon/bird crest, noble tunic with central crest depicting the salt valley landscape, and the celebrated medieval battleaxe held firmly in his right hand.'
    },
    ball: {
      ca: 'Ball de Gegants de Sant Miquel i Pasdoble del Batallador',
      es: 'Baile de Gigantes de San Miguel y Pasodoble del Batallador',
      en: 'Sant Miquel Giants Dance and Batallador Pasodoble'
    },
    badge: {
      ca: 'Gegant Guerrer (1957)',
      es: 'Gigante Guerrero (1957)',
      en: 'Warrior Giant (1957)'
    },
    featured: true
  },
  {
    id: 'esperanca',
    name: 'L\'Esperança',
    subtitle: {
      ca: 'Geganta del Barri de Sant Miquel',
      es: 'Giganta del Barrio de San Miguel',
      en: 'Giantess of the Sant Miquel Quarter'
    },
    category: 'sant-miquel',
    categoryLabel: {
      ca: 'Barri de Sant Miquel',
      es: 'Barrio de San Miguel',
      en: 'Sant Miquel Quarter'
    },
    year: {
      ca: '1957 (Estrena: 16 de juny de 1957)',
      es: '1957 (Estreno: 16 de junio de 1957)',
      en: '1957 (Debut: June 16, 1957)'
    },
    sculptor: {
      ca: 'El Ingenio (Barcelona) · Restauracions de Manel Casserres i Toni Mujal',
      es: 'El Ingenio (Barcelona) · Restauraciones de Manel Casserres y Toni Mujal',
      en: 'El Ingenio (Barcelona) · Restorations by Manel Casserres & Toni Mujal'
    },
    height: '3,70 m',
    weight: '55 kg',
    location: {
      ca: 'Cardona - Barri de Sant Miquel',
      es: 'Cardona - Barrio de San Miguel',
      en: 'Cardona - Sant Miquel Quarter'
    },
    image: './images/esperanca.jpg',
    imagePosition: 'center 48%',
    description: {
      ca: 'La geganta Esperança és la dama senyorial del Barri de Sant Miquel de Cardona i parella inseparable d\'El Batallador. Estrenada solemnement el 16 de juny de 1957 gràcies a la iniciativa veïnal finançada per Jaume Vendrell Rosas, simbolitza la concòrdia, la bellesa i la virtut que acompanyen el caràcter heroic del guerrer cardoní.',
      es: 'La giganta Esperança es la dama señorial del Barrio de San Miguel de Cardona y pareja inseparable de El Batallador. Estrenada solemnemente el 16 de junio de 1957 gracias a la iniciativa vecinal financiada por Jaume Vendrell Rosas, simboliza la concordia, la belleza y la virtud que acompañan el carácter heroico del guerrero cardonino.',
      en: 'The Esperança giantess is the stately lady of Cardona’s Sant Miquel quarter and inseparable companion to El Batallador. Solemnly unveiled on June 16, 1957 through neighborly initiative funded by Jaume Vendrell Rosas, she symbolizes harmony, beauty, and virtue complementing the warrior’s valor.'
    },
    vestimenta: {
      ca: 'Majestuosa corona reial daurada sobre cabellera castanya ondulada, arracades aristocràtiques de pedreria, mantell verd oliva amb brocats florals a les espatlles, vestit de brocat daurat amb sanefa triangular gòtica i estel a la faldilla. Lluu al pit l\'escut heràldic de la vall salina i sosté a la mà un delicat ram de flors silvestres davant els balcons engalanats de la vila.',
      es: 'Majestuosa corona real dorada sobre cabellera castaña ondulada, pendientes aristocráticos de pedrería, manto verde oliva con brocados florales en los hombros, vestido de brocado dorado con cenefa triangular gótica y estrella en la falda. Luce en el pecho el escudo heráldico del valle salino y sostiene en la mano un delicado ramo de flores silvestres ante los balcones engalanados de la villa.',
      en: 'Majestic golden royal crown upon wavy chestnut hair, aristocratic gem earrings, olive-green cloak with floral brocades over the shoulders, and a golden brocade gown with Gothic triangular pattern and star emblem. On her breast rests the heraldic crest of the salt valley and she gracefully carries a bouquet of wildflowers.'
    },
    ball: {
      ca: 'Ball de Gegants de Sant Miquel i Vals de l\'Esperança',
      es: 'Baile de Gigantes de San Miguel y Vals de la Esperança',
      en: 'Sant Miquel Giants Dance and Esperança Waltz'
    },
    badge: {
      ca: 'Geganta de Sant Miquel (1957)',
      es: 'Giganta de San Miguel (1957)',
      en: 'Sant Miquel Giantess (1957)'
    },
    featured: true
  },
  {
    id: 'borrell-ii',
    name: 'Borrell II',
    subtitle: {
      ca: 'Comte de Barcelona i Senyor de Cardona',
      es: 'Conde de Barcelona y Señor de Cardona',
      en: 'Count of Barcelona and Lord of Cardona'
    },
    category: 'barri-major',
    categoryLabel: {
      ca: 'Barri Major (Centenaris)',
      es: 'Barrio Mayor (Centenarios)',
      en: 'Barri Major (Centenary Giants)'
    },
    year: {
      ca: '1834 (batejat el 1984)',
      es: '1834 (bautizado en 1984)',
      en: '1834 (baptized in 1984)'
    },
    sculptor: {
      ca: 'Deogràcies Valls (restauracions de Toni Mujal)',
      es: 'Deogràcies Valls (restauraciones de Toni Mujal)',
      en: 'Deogràcies Valls (restorations by Toni Mujal)'
    },
    height: '3,85 m',
    weight: '62 kg',
    location: {
      ca: 'Cardona - Barri Major',
      es: 'Cardona - Barrio Mayor',
      en: 'Cardona - Barri Major'
    },
    image: './images/borrell.jpg',
    description: {
      ca: 'El gegant del Barri Major representa el poder noble i comtal de la Cardona medieval. Borrell II és el comte que atorgà la cèlebre Carta de Poblament l\'any 986 a l\'església de Sant Vicenç del Castell de Cardona, concedint franquícies als pobladors del castell i la vall salina.',
      es: 'El gigante del Barrio Mayor representa el poder noble y condal de la Cardona medieval. Borrell II es el conde que otorgó la célebre Carta de Población en el año 986 en la iglesia de San Vicente del Castillo de Cardona, concediendo libertades a los pobladores del castillo y el valle salino.',
      en: 'The Barri Major giant represents the noble ducal heritage of medieval Cardona. Borrell II is the Count who granted the renowned founding charter in 986 at the Church of Sant Vicenç in Cardona Castle, granting privileges to the settlers of the fortress and salt valley.'
    },
    vestimenta: {
      ca: 'Toc cerimonial en tons blancs i verds amb joia d\'or frontal, cabellera negra i bigoti noble, corbatí de llaç i túnica comtal amb brocats daurats. Lluu al pit la gran medalla commemorativa de la Vila de Cardona penjada d\'un cordó trenat i mantell senyorial davant les muralles del Castell.',
      es: 'Tocado ceremonial en tonos blancos y verdes con joya de oro frontal, cabellera negra y bigote noble, corbatín de lazo y túnica condal con brocados dorados. Luce en el pecho la gran medalla conmemorativa de la Villa de Cardona colgada de un cordón trenzado y manto señorial ante las murallas del Castillo.',
      en: 'Ceremonial headwear in green and white tones with a central gold jewel, dark hair and noble mustache, bow tie, and ducal tunic with golden brocades. On his chest hangs the large commemorative medal of Cardona on a braided cord, complemented by a stately mantle.'
    },
    ball: {
      ca: 'Ball de Gegants de Cardona i Pasdoble Major',
      es: 'Baile de Gigantes de Cardona y Pasodoble Mayor',
      en: 'Cardona Giants Dance and Major Pasodoble'
    },
    badge: {
      ca: 'Centenari (1834)',
      es: 'Centenario (1834)',
      en: 'Centenary Giant (1834)'
    },
    featured: true
  },
  {
    id: 'letgarda',
    name: 'Letgarda',
    subtitle: {
      ca: 'Comtessa consort de Cardona',
      es: 'Condesa consorte de Cardona',
      en: 'Countess consort of Cardona'
    },
    category: 'barri-major',
    categoryLabel: {
      ca: 'Barri Major (Centenaris)',
      es: 'Barrio Mayor (Centenarios)',
      en: 'Barri Major (Centenary Giants)'
    },
    year: {
      ca: '1834 (batejada el 1984)',
      es: '1834 (bautizada en 1984)',
      en: '1834 (baptized in 1984)'
    },
    sculptor: {
      ca: 'Deogràcies Valls (restauracions de Toni Mujal)',
      es: 'Deogràcies Valls (restauraciones de Toni Mujal)',
      en: 'Deogràcies Valls (restorations by Toni Mujal)'
    },
    height: '3,75 m',
    weight: '58 kg',
    location: {
      ca: 'Cardona - Barri Major',
      es: 'Cardona - Barrio Mayor',
      en: 'Cardona - Barri Major'
    },
    image: './images/letgarda.jpg',
    description: {
      ca: 'Parella inseparable de Borrell II. Simbolitza l\'elegància i sobirania medieval de Cardona. Juntament amb Borrell II, van ser construïts en fusta, cartró pedra i vímet a principis del segle XIX, sent de les figures més antigues i estimades de tot el Bages.',
      es: 'Pareja inseparable de Borrell II. Simboliza la elegancia y soberanía medieval de Cardona. Junto con Borrell II, fueron construidos en madera, cartón piedra y mimbre a principios del siglo XIX, siendo de las figuras más antiguas y queridas de toda la comarca del Bages.',
      en: 'Inseparable companion of Borrell II. She embodies the medieval elegance and sovereignty of Cardona. Carved from wood, papier-mâché and wicker in the early 19th century, they are among the oldest and most beloved giant figures in Catalonia.'
    },
    vestimenta: {
      ca: 'Pentinat noble d\'època amb recollit trenat i ornaments florals, arracades aristocràtiques de pedreria fina, vestit de gala en to blau teixit amb randa de puntes blanques al coll i espatlles, camisa prisada d\'ivori i mocador d\'encaix tradicional, alçada amb elegància davant la muralla del Castell de Cardona.',
      es: 'Peinado noble de época con recogido trenzado y adornos florales, pendientes aristocráticos de pedrería fina, vestido de gala en tono azul tejido con puntillas blancas en cuello y hombros, camisa plisada de marfil y pañuelo de encaje tradicional, erguida con elegancia ante la muralla del Castillo de Cardona.',
      en: 'Noble period coiffure with braided updo and floral accents, delicate aristocratic earrings, gala gown in royal blue with white lace edging at neckline and shoulders, ivory pleated blouse, and traditional lace handkerchief.'
    },
    ball: {
      ca: 'Ball Pla i Marxa dels Gegants del Barri Major',
      es: 'Ball Pla y Marcha de los Gigantes del Barrio Mayor',
      en: 'Ball Pla and Barri Major Processional March'
    },
    badge: {
      ca: 'Centenària (1834)',
      es: 'Centenaria (1834)',
      en: 'Centenary Giantess (1834)'
    },
    featured: true
  },
  {
    id: 'romeu',
    name: 'Romeu',
    subtitle: {
      ca: 'Gegant Centenari del Barri de la Fira',
      es: 'Gigante Centenario del Barrio de la Fira',
      en: 'Centenary Giant of the Fira Quarter'
    },
    category: 'barri-fira',
    categoryLabel: {
      ca: 'Barri de la Fira (Centenari)',
      es: 'Barrio de la Fira (Centenario)',
      en: 'Fira Quarter (Centenary Giant)'
    },
    year: {
      ca: '1908 (Estrena: Barri de la Fira)',
      es: '1908 (Estreno: Barrio de la Fira)',
      en: '1908 (Debut: Fira Quarter)'
    },
    sculptor: {
      ca: 'Miquel Serra i Mosella ("Sabater Fraret")',
      es: 'Miquel Serra i Mosella ("Sabater Fraret")',
      en: 'Miquel Serra i Mosella ("Sabater Fraret")'
    },
    height: '3,20 m',
    weight: '48 kg',
    location: {
      ca: 'Cardona - Barri de la Fira',
      es: 'Cardona - Barrio de la Fira',
      en: 'Cardona - Fira Quarter'
    },
    image: './images/romeu.jpg',
    description: {
      ca: 'El Gegant Romeu és la segona figura més antiga de Cardona, construït el 1908 pel cèlebre sabater cardoní Miquel Serra («Sabater Fraret») per a les festivitats del Barri de la Fira. Forma la mítica parella de «Romeu i Julieta» i està catalogat oficialment com a Gegant Centenari de Catalunya. La seva mirada expressiva i somriure noble el fan una de les figures més carismàtiques de la vila.',
      es: 'El Gigante Romeu es la segunda figura más antigua de Cardona, construido en 1908 por el célebre zapatero cardonino Miquel Serra («Sabater Fraret») para las fiestas del Barrio de la Fira. Forma la mítica pareja de «Romeu y Julieta» y está catalogado oficialmente como Gigante Centenario de Cataluña. Su mirada expresiva y sonrisa noble lo convierten en una de las figuras más carismáticas de la villa.',
      en: 'Romeu is the second oldest giant in Cardona, crafted in 1908 by local shoemaker Miquel Serra ("Sabater Fraret") for the Fira quarter festivities. He forms the mythical pair "Romeu and Julieta" and is officially registered as a Centenary Giant of Catalonia. His expressive gaze and noble smile make him one of the town’s most charismatic figures.'
    },
    vestimenta: {
      ca: 'Barret cerimonial de feltre carmesí amb ploma noble i fermall daurat, cabellera negra i bigoti corbat, capa de vellut grana amb sanefes i túnica ivori. Lluu al pit amb gran orgull l\'escut oficial de Cardona: les quatre barres catalanes amb la mata de card comtal al centre.',
      es: 'Sombrero ceremonial de fieltro carmesí con pluma noble y broche dorado, cabellera negra y bigote curvado, capa de terciopelo granate con cenefas y túnica marfil. Luce en el pecho con gran orgullo el escudo oficial de Cardona: las cuatro barras catalanas con el cardo condal en el centro.',
      en: 'Ceremonial crimson felt hat with noble feather and gold brooch, dark hair, curved mustache, garnet velvet cape, and ivory tunic. He proudly bears the official Cardona crest on his chest: the Catalan red-and-gold stripes with the thistle at its heart.'
    },
    ball: {
      ca: 'Vals de Romeu i Julieta i Ballada del Barri de la Fira',
      es: 'Vals de Romeu y Julieta y Baile del Barrio de la Fira',
      en: 'Romeu & Julieta Waltz and Fira Dance'
    },
    badge: {
      ca: 'Centenari (1908)',
      es: 'Centenario (1908)',
      en: 'Centenary Giant (1908)'
    },
    featured: true
  },
  {
    id: 'julieta',
    name: 'Julieta',
    subtitle: {
      ca: 'Geganta Centenària del Barri de la Fira',
      es: 'Giganta Centenaria del Barrio de la Fira',
      en: 'Centenary Giantess of the Fira Quarter'
    },
    category: 'barri-fira',
    categoryLabel: {
      ca: 'Barri de la Fira (Centenària)',
      es: 'Barrio de la Fira (Centenaria)',
      en: 'Fira Quarter (Centenary Giantess)'
    },
    year: {
      ca: '1908 (Estrena: Barri de la Fira)',
      es: '1908 (Estreno: Barrio de la Fira)',
      en: '1908 (Debut: Fira Quarter)'
    },
    sculptor: {
      ca: 'Miquel Serra i Mosella ("Sabater Fraret")',
      es: 'Miquel Serra i Mosella ("Sabater Fraret")',
      en: 'Miquel Serra i Mosella ("Sabater Fraret")'
    },
    height: '3,15 m',
    weight: '44 kg',
    location: {
      ca: 'Cardona - Barri de la Fira',
      es: 'Cardona - Barrio de la Fira',
      en: 'Cardona - Fira Quarter'
    },
    image: './images/julieta.jpg',
    imagePosition: 'center 12%',
    description: {
      ca: 'La Geganta Julieta és la mítica companya del gegant Romeu i una de les figures centenàries més emblemàtiques i estimades de Cardona. Construïda l\'any 1908 pel sabater artesà Miquel Serra («Sabater Fraret») per a les festivitats del Barri de la Fira, està catalogada oficialment com a Geganta Centenària de Catalunya. Amb els seus ulls blaus expressius, mirada dolça i somriure afable, encarna la donzella noble i romàntica que enamora el poble a cada ballada de festa major.',
      es: 'La Giganta Julieta es la mítica compañera del gigante Romeu y una de las figuras centenarias más emblemáticas y queridas de Cardona. Construida en el año 1908 por el zapatero artesano Miquel Serra («Sabater Fraret») para las festividades del Barrio de la Fira, está catalogada oficialmente como Giganta Centenaria de Cataluña. Con sus expresivos ojos azules, dulce mirada y sonrisa afable, encarna a la doncella noble y romántica que cautiva a la villa en cada baile de fiesta mayor.',
      en: 'Julieta is the legendary companion of giant Romeu and one of Cardona’s most cherished centenary figures. Created in 1908 by local craftsman and shoemaker Miquel Serra ("Sabater Fraret") for the Fira quarter celebrations, she is officially registered as a Centenary Giantess of Catalonia. With her expressive blue eyes, gentle gaze, and sweet countenance, she personifies the noble romantic heroine that enchants the town during every festival dance.'
    },
    vestimenta: {
      ca: 'Recollit noble d\'època ornat amb dues roses al capdamunt (una grana i una argentada), collaret de perles blanques i arracades a joc. Llueix un vestit senyorial amb cosset central de vellut carmesí decorat amb passamaneria platejada i calat de reixa inferior, combinat amb una rica tela adamascada platejada amb motius florals a les àmplies mànegues i faldilla. Porta a la cintura una bossa almoinera de gala a joc, i a la mà dreta sosté amb delicadesa un ramet de flors silvestres morades (lavanda) embolicat en tul violeta i cintes de la senyera.',
      es: 'Elegante recogido noble con dos rosas en el tocado (una granate y otra plateada), collar de perlas blancas y pendientes a juego. Luce un vestido señorial con corpiño central de terciopelo carmesí decorado con cordoncillo plateado y calado de rejilla inferior, combinado con una rica tela adamascada plateada con motivos florales en las amplias mangas y falda. Lleva en la cintura una limosnera de gala a juego, y en la mano derecha sostiene con delicadeza un ramo de flores silvestres moradas (lavanda) envuelto en tul violeta y cintas de la senyera.',
      en: 'Noble period updo adorned with a garnet and silver dual-rose headpiece, white pearl necklace, and matching drop earrings. She wears a stately gown with a crimson velvet central bodice decorated with silver cord trims and lower grid openwork, paired with silver-grey floral damask jacquard on the sweeping sleeves and skirt. A matching gala alms-purse hangs from her waist, and in her right hand she delicately holds a bouquet of purple lavender wrapped in violet tulle and Catalan senyera ribbons.'
    },
    ball: {
      ca: 'Vals de la Fira (Romeu i Julieta)',
      es: 'Vals de la Fira (Romeu y Julieta)',
      en: 'Fira Waltz (Romeu & Julieta)'
    },
    badge: {
      ca: 'Centenària (1908)',
      es: 'Centenaria (1908)',
      en: 'Centenary Giantess (1908)'
    },
    featured: true
  },
  {
    id: 'abdalla',
    name: 'Abdal·là',
    subtitle: {
      ca: 'Príncep Sarraí / Barri Nou',
      es: 'Príncipe Sarraceno / Barrio Nuevo',
      en: 'Saracen Prince / Barri Nou'
    },
    category: 'barri-nou',
    categoryLabel: {
      ca: 'Barri Nou',
      es: 'Barrio Nuevo',
      en: 'Barri Nou Quarter'
    },
    year: {
      ca: '1980 (renovació Toni Mujal, 2019)',
      es: '1980 (renovación Toni Mujal, 2019)',
      en: '1980 (renewed by Toni Mujal, 2019)'
    },
    sculptor: {
      ca: 'Toni Mujal (taller cardoní)',
      es: 'Toni Mujal (taller cardonino)',
      en: 'Toni Mujal (Cardona workshop)'
    },
    height: '3,90 m',
    weight: '64 kg',
    location: {
      ca: 'Cardona - Barri Nou',
      es: 'Cardona - Barrio Nuevo',
      en: 'Cardona - Barri Nou'
    },
    image: './images/abdalla.jpg',
    description: {
      ca: 'Protagonista de la immortal llegenda de la Torre de la Minyona del Castell de Cardona. Abdal·là, noble i valent príncep sarraí, va caure presoner d\'amor per la jove Adalés, la filla del vescomte de Cardona, desencadenant una de les històries d\'amor i tragèdia més profundes i estimades del patrimoni popular cardoní.',
      es: 'Protagonista de la inmortal leyenda de la Torre de la Minyona del Castillo de Cardona. Abdal·là, noble y valiente príncipe sarraceno, cayó prisionero de amor por la joven Adalés, hija del vizconde de Cardona, desencadenando una de las historias de amor y tragedia más profundas y queridas del patrimonio popular de Cardona.',
      en: 'Hero of the immortal legend of the Minyona Tower at Cardona Castle. Abdal·là, a noble and valiant Saracen prince, fell deeply in love with young Adalés, daughter of the Viscount of Cardona, sparking one of the most poignant romantic legends in Catalan folklore.'
    },
    vestimenta: {
      ca: 'Casc metàl·lic de guerrer amb turbant d\'or i teixits nobles, cota de malla i armadura daurada sobre túnica carmesí. Empunya amb fermesa a la mà dreta una imponent espasa corba o simitarra sarraïna, i porta al braç esquerre un gran escut rodó fosc ornat amb la mitja lluna daurada en relleu, dret i senyorial davant els murs del Castell de Cardona.',
      es: 'Casco metálico de guerrero con turbante de oro y tejidos nobles, cota de malla y armadura dorada sobre túnica carmesí. Empuña con firmeza en su mano derecha una imponente espada curva o cimitarra sarracena, y lleva en el brazo izquierdo un gran escudo redondo oscuro adornado con la media luna dorada en relieve, erguido y señorial ante los muros del Castillo de Cardona.',
      en: 'Warrior metal helm with golden turban and noble fabrics, chainmail and gold armor over a crimson tunic. He holds firmly in his right hand an imposing curved scimitar, and carries on his left arm a large round dark shield emblazoned with a golden crescent in relief.'
    },
    ball: {
      ca: 'Dansa Morisca i Ball dels Gegants Nous',
      es: 'Danza Morisca y Baile de los Gigantes Nuevos',
      en: 'Moorish Dance and New Giants Dance'
    },
    badge: {
      ca: 'Llegenda del Castell',
      es: 'Leyenda del Castillo',
      en: 'Castle Legend'
    },
    featured: true
  },
  {
    id: 'adales',
    name: 'Adalés',
    subtitle: {
      ca: 'La Minyona de Cardona / Barri Nou',
      es: 'La Minyona de Cardona / Barrio Nuevo',
      en: 'The Minyona of Cardona / Barri Nou'
    },
    category: 'barri-nou',
    categoryLabel: {
      ca: 'Barri Nou',
      es: 'Barrio Nuevo',
      en: 'Barri Nou Quarter'
    },
    year: {
      ca: '1980 (renovació Toni Mujal, 2019)',
      es: '1980 (renovación Toni Mujal, 2019)',
      en: '1980 (renewed by Toni Mujal, 2019)'
    },
    sculptor: {
      ca: 'Toni Mujal (taller cardoní)',
      es: 'Toni Mujal (taller cardonino)',
      en: 'Toni Mujal (Cardona workshop)'
    },
    height: '3,78 m',
    weight: '59 kg',
    location: {
      ca: 'Cardona - Barri Nou',
      es: 'Cardona - Barrio Nuevo',
      en: 'Cardona - Barri Nou'
    },
    image: './images/adales.jpg',
    description: {
      ca: 'La donzella Adalés (coneguda també en la tradició com la Minyona de Cardona) és la noble dama del Barri Nou i protagonista de la cèlebre llegenda de la Torre de la Minyona del Castell de Cardona. Filla del vescomte de Cardona, el seu amor secret pel príncep sarraí Abdal·là va desafiar les rivalitats de l\'època. Avui dia, tots dos gegants dansen lliures i units als carrers i places de la vila com a màxim símbol d\'amor, llibertat i concòrdia.',
      es: 'La doncella Adalés (conocida también en la tradición como la Minyona de Cardona) es la noble dama del Barrio Nuevo y protagonista de la célebre leyenda de la Torre de la Minyona del Castillo de Cardona. Hija del vizconde de Cardona, su amor secreto por el príncipe sarraceno Abdal·là desafió las rivalidades de la época. Hoy en día, ambos gigantes danzan libres y unidos en las calles y plazas de la villa como máximo símbolo de amor, libertad y concordia.',
      en: 'The maiden Adalés (traditionally known as the Minyona of Cardona) is the noble lady of Barri Nou and heroine of the celebrated Torre de la Minyona legend. Daughter of the Viscount of Cardona, her secret romance with Saracen prince Abdal·là defied contemporary rivalries. Today, both giants dance freely reunited in Cardona’s streets as a supreme symbol of love, freedom, and fellowship.'
    },
    vestimenta: {
      ca: 'Pentinat clàssic amb llargues trenes castanyes daurades sobre les espatlles enllaçades amb vetes blanques i penjoll noble al coll. Llueix un vestit medieval d\'època en to verd maragda i vellut oliva amb amples mànigues acampanades, cosset ajustat amb sanefes d\'or i sanefa geomètrica de rombes a la faldilla. Sosté a la mà dreta un llibre enquadernat en cuir antic, capturada al costat d\'Abdal·là davant el portal gòtic de Sant Miquel de Cardona.',
      es: 'Peinado clásico con largas trenzas castañas doradas sobre los hombros entrelazadas con cintas blancas y colgante noble en el cuello. Luce un vestido medieval de época en tono verde esmeralda y terciopelo oliva con amplias mangas acampanadas, corpiño ajustado con cenefas de oro y cenefa geométrica de rombos en la falda. Sostiene en la mano derecha un libro encuadernado en cuero antiguo, junto a Abdal·là ante el portal gótico de San Miguel de Cardona.',
      en: 'Classic hairstyle featuring long golden-chestnut braids over her shoulders entwined with white ribbons and a noble pendant necklace. She wears an emerald-green and olive-velvet medieval dress with bell sleeves, fitted bodice with golden trim, and diamond-patterned skirt hem, holding an ancient leather-bound book in her right hand.'
    },
    ball: {
      ca: 'Dansa d\'Adalés i Abdal·là i Vals del Barri Nou',
      es: 'Danza de Adalés y Abdal·là y Vals del Barrio Nuevo',
      en: 'Dance of Adalés & Abdal·là and Barri Nou Waltz'
    },
    badge: {
      ca: 'Llegenda del Castell',
      es: 'Leyenda del Castillo',
      en: 'Castle Legend'
    },
    featured: true
  },
  {
    id: 'dimoni-raval',
    name: 'El Dimoni del Raval',
    subtitle: {
      ca: 'Gegant del Raval de Sant Joan / Pont del Diable',
      es: 'Gigante del Raval de San Juan / Puente del Diablo',
      en: 'Giant of Raval de Sant Joan / Devil\'s Bridge'
    },
    category: 'barri-raval',
    categoryLabel: {
      ca: 'Barri del Raval (Sant Joan)',
      es: 'Barrio del Raval (San Juan)',
      en: 'Raval Quarter (Sant Joan)'
    },
    year: {
      ca: '2009 (Estrena: Festes del Raval)',
      es: '2009 (Estreno: Fiestas del Raval)',
      en: '2009 (Debut: Raval Festivities)'
    },
    sculptor: {
      ca: 'Toni Mujal (mestre geganter cardoní)',
      es: 'Toni Mujal (maestro constructor cardonino)',
      en: 'Toni Mujal (Cardona master craftsman)'
    },
    height: '3,60 m',
    weight: '50 kg',
    location: {
      ca: 'Cardona - Raval de Sant Joan',
      es: 'Cardona - Raval de San Juan',
      en: 'Cardona - Raval de Sant Joan'
    },
    image: './images/dimoni-raval.jpg',
    imagePosition: 'center 10%',
    description: {
      ca: 'Figura emblemàtica i carismàtica del Barri del Raval de Sant Joan de Cardona, construïda el 2009 pel mestre geganter cardoní Toni Mujal. Encarna el personatge de la cèlebre llegenda popular del Pont del Diable —monumental pont gòtic medieval inacabat del segle XIV sobre el riu Cardener que, segons la tradició oral, el dimoni va pactar aixecar en una sola nit abans que cantés el gall. Com a homenatge entranyable, el seu rostre rialler i expressiu amb ulleres va ser modelat a semblança d\'un veí molt estimat del barri.',
      es: 'Figura emblemática y carismática del Barrio del Raval de San Juan de Cardona, construida en 2009 por el maestro de gigantes local Toni Mujal. Encarna al personaje de la célebre leyenda popular del Puente del Diablo —monumental puente gótico medieval inconcluso del siglo XIV sobre el río Cardener que, según la tradición oral, el diablo pactó levantar en una sola noche antes de que cantara el gallo. Como entrañable homenaje vecinal, su rostro risueño y expresivo con gafas fue modelado a semejanza de un vecino muy querido del barrio.',
      en: 'An iconic and charismatic figure of Cardona’s Raval de Sant Joan quarter, created in 2009 by renowned Cardona master craftsman Toni Mujal. He embodies the devil from the local legend of the Pont del Diable (Devil’s Bridge) — the monumental 14th-century unfinished Gothic bridge over the Cardener river which, according to folklore, the devil pledged to build in a single night before the cock crowed. In an affectionate community tribute, his jovial bespectacled face was sculpted after a beloved local neighbor.'
    },
    vestimenta: {
      ca: 'Elegància diabòlica singular: banyes fosques, orelles punxegudes i ulleres de muntura metàl·lica amb perilla riallera. Llueix frac i americana negra folrada de domàs vermell viu amb acabats en forma de flames, capa negra amb coll alt rígid en punta estil ales de ratpenat forrat en vermell, camisa blanca amb botons i llaç negre cordat, faldó de domàs vermell amb motius florals, gran anell daurat amb segell a la mà dreta i un imponent trident o forca de tres puntes argentada que sosté amb fermesa.',
      es: 'Elegancia diabólica singular: cuernos oscuros, orejas puntiagudas y gafas de montura metálica con perilla sonriente. Viste frac y chaqueta negra forrada de damasco rojo vivo con solapas en forma de llamas, capa negra con cuello alto rígido en punta estilo alas de murciélago forrado en rojo, camisa blanca con botones y lazo negro cordado, faldón de damasco rojo con motivos florales, gran anillo dorado con sello en la mano derecha y un imponente tridente o forca plateada de tres puntas que sostiene con firmeza.',
      en: 'A uniquely dapper devil: dark curved horns, pointed ears, wire-rimmed spectacles, and a cheerful goatee. He wears a black tuxedo jacket with flame-scalloped red damask lapels, a black cape with a high batwing-style crimson collar, crisp white buttoned shirt with a corded black bow tie, a rich red floral damask skirt, a large gold signet ring on his right hand, and a towering silver three-pronged pitchfork held firmly aloft.'
    },
    ball: {
      ca: 'Ball del Dimoni del Raval i Vals del Pont del Diable',
      es: 'Baile del Dimoni del Raval y Vals del Puente del Diablo',
      en: 'Dance of the Raval Devil & Devil’s Bridge Waltz'
    },
    badge: {
      ca: 'Llegenda del Pont (2009)',
      es: 'Leyenda del Puente (2009)',
      en: 'Bridge Legend (2009)'
    },
    featured: true
  },
  {
    id: 'gegants-coromina',
    name: 'Gegants de la Coromina',
    subtitle: {
      ca: 'Joan de Serrallonga i Margarida (Barri de la Coromina)',
      es: 'Joan de Serrallonga y Margarida (Barrio de la Coromina)',
      en: 'Joan de Serrallonga & Margarida (La Coromina Quarter)'
    },
    category: 'barri-coromina',
    categoryLabel: {
      ca: 'Barri de la Coromina',
      es: 'Barrio de la Coromina',
      en: 'La Coromina Quarter'
    },
    year: {
      ca: '1969-1970 (Estrena oficial: Festa Major de la Coromina 1970)',
      es: '1969-1970 (Estreno oficial: Fiesta Mayor de la Coromina 1970)',
      en: '1969-1970 (Official debut: La Coromina Festival 1970)'
    },
    sculptor: {
      ca: 'Manel Casserres i Boix (plànols, caps i mans) i iniciativa veïnal · Modistes: Paquita Muixí i Àngela Godino',
      es: 'Manel Casserres i Boix (planos, cabezas y manos) e iniciativa vecinal · Modistas: Paquita Muixí y Àngela Godino',
      en: 'Manel Casserres i Boix (design, heads & hands) & local neighborhood initiative · Dressmakers: Paquita Muixí & Àngela Godino'
    },
    height: '3,55 m (Joan) / 3,45 m (Margarida)',
    weight: '52 kg / 48 kg',
    location: {
      ca: 'Cardona - Barri de la Coromina',
      es: 'Cardona - Barrio de la Coromina',
      en: 'Cardona - La Coromina Quarter'
    },
    image: './images/gegants-coromina.jpg',
    imagePosition: 'center 10%',
    description: {
      ca: 'Els Gegants de la Coromina representen la llegendària figura del cèlebre bandoler català del segle XVII Joan de Serrallonga i la seva muller, Margarida de Serrallonga (també citada com a Leonor de Sicília en algunes fonts). Construïts entre 1969 i 1970 per iniciativa i recaptació popular dels veïns del barri miner i obrer de la Coromina, van comptar amb la mestria de l\'escultor solsoní Manel Casserres i Boix —qui en va modelar els caps i les mans prenent com a referència els històrics Gegants del Pi de Barcelona. Es van presentar oficialment el diumenge de la Festa Major de la Coromina de 1970 acompanyats pel Drac i el Bou, i han celebrat més de mig segle de vida aportant caràcter, orgull i tradició popular a les festes de Cardona.',
      es: 'Los Gigantes de la Coromina representan la mítica figura del célebre bandolero catalán del siglo XVII Joan de Serrallonga y su esposa, Margarida de Serrallonga (también citada como Leonor de Sicilia en algunas fuentes). Construidos entre 1969 y 1970 por iniciativa y suscripción popular de los vecinos del barrio minero y obrero de la Coromina, contaron con la maestría del escultor solsonense Manel Casserres i Boix —quien modeló las cabezas y manos tomando como referencia los históricos Gigantes del Pi de Barcelona. Se presentaron oficialmente el domingo de la Fiesta Mayor de la Coromina de 1970 acompañados por el Dragón y el Buey, y han celebrado más de medio siglo de vida aportando carácter, arraigo y tradición popular a las fiestas de Cardona.',
      en: 'The Giants of La Coromina depict the legendary 17th-century Catalan bandit Joan de Serrallonga and his wife Margarida de Serrallonga (also referred to as Leonor de Sicília in some records). Crafted between 1969 and 1970 through the fundraising initiative of the mining community of La Coromina, they were created with the guidance of master sculptor Manel Casserres i Boix from Solsona, who sculpted their heads and hands inspired by the historic Pi Giants of Barcelona. They officially debuted at the 1970 Coromina Festival alongside the Dragon and the Bull, celebrating more than fifty years of proud festive tradition in Cardona.'
    },
    vestimenta: {
      ca: 'Indumentària tradicional catalana renovada per al seu 50è aniversari (2021). En Joan llueix barretina vermella, barba i bigoti foscos, camisa blanca amb coll obert, armilla de vellut carmesí amb vius vermells i tancaments tradicionals de passamaneria, faixa vermella viva, corretja de bandoler a la mà dreta i faldó de vellut amb espieta. La Margarida llueix pentinat recollit amb gandalla i llaç de vellut negre, arracades vermelles de gota, cosset d\'estampat floral primaveral, gran mantó de puntes i blonda blanca amb serrells subjectat amb fermall d\'orfebreria antiga, davantal de vellut negre amb rivet calat, mitenes negres als braços, faldilla prisada verd fosc i ramet de flors silvestres seques i espígol.',
      es: 'Indumentaria tradicional catalana renovada con motivo de su 50 aniversario (2021). Joan luce barretina roja, barba y bigote oscuros, camisa blanca con cuello abierto, chaleco de terciopelo carmesí con vivos rojos y botones tradicionales de pasamanería, faja roja brillante, correa en mano derecha y faldón de terciopelo con mirilla. Margarida luce cabello recogido con redecilla y lazo de terciopelo negro, pendientes rojos en gota, corpiño de estampado floral primaveral, gran mantón de encaje y blonda blanca con flecos cerrado con broche de orfebrería antigua, delantal de terciopelo negro ribeteado, mitones negros en los brazos, falda plisada verde oscuro y ramo de flores silvestres y lavanda.',
      en: 'Traditional Catalan festive attire renewed for their 50th anniversary (2021). Joan wears a classic red barretina cap, dark beard and mustache, open-collar white shirt, crimson velvet waistcoat with vibrant red piping and passementerie fasteners, bright red sash, strap in hand, and velvet gown with viewing grille. Margarida features hair gathered in a black velvet snood, red drop earrings, floral bodice, exquisite white lace fringe shawl secured with an antique brooch, black velvet apron, black lace fingerless mitts, pleated dark green skirt, and a wildflower and lavender bouquet.'
    },
    ball: {
      ca: 'Ball de Gegants de la Coromina (Música pròpia composta pel veí Antoni Ribas)',
      es: 'Baile de Gigantes de la Coromina (Música propia compuesta por el vecino Antoni Ribas)',
      en: 'La Coromina Giants Dance (Original music composed by local neighbor Antoni Ribas)'
    },
    badge: {
      ca: 'Joan i Margarida (1969)',
      es: 'Joan y Margarida (1969)',
      en: 'Joan & Margarida (1969)'
    },
    featured: true
  },
  {
    id: 'nans-mercat',
    name: 'Nans del Mercat (Minga i Agneta)',
    subtitle: {
      ca: 'Els Nans més antics de Cardona (Barri del Mercat)',
      es: 'Los Cabezudos más antiguos de Cardona (Barrio del Mercado)',
      en: 'The Oldest Dwarf Figures of Cardona (Market Quarter)'
    },
    category: 'capgrossos',
    categoryLabel: {
      ca: 'Nans i Capgrossos (Barri del Mercat)',
      es: 'Cabezudos y Enanos (Barrio del Mercado)',
      en: 'Dwarfs & Big Heads (Market Quarter)'
    },
    year: {
      ca: 'Segle XVIII (ca. 1750-1800) · Batejats el 1996',
      es: 'Siglo XVIII (ca. 1750-1800) · Bautizados en 1996',
      en: '18th Century (c. 1750-1800) · Baptized in 1996'
    },
    sculptor: {
      ca: 'Artesans tradicionals (Casa Climens) · Rèpliques de 1996 per a conservació',
      es: 'Artesanos tradicionales (Casa Climens) · Réplicas de 1996 para conservación',
      en: 'Traditional Craftsmen (Casa Climens) · 1996 Conservation Replicas'
    },
    height: '1,80 m',
    weight: '12 kg (cada nan)',
    location: {
      ca: 'Cardona - Barri del Mercat',
      es: 'Cardona - Barrio del Mercado',
      en: 'Cardona - Market Quarter'
    },
    image: './images/nans-mercat.jpg',
    description: {
      ca: 'Els Nans del Mercat són la parella de figures còmiques més antiga de Cardona i una de les més antigues de tot Catalunya, amb orígens documentats que es remunten a la segona meitat del segle XVIII (1750-1800). Vinculats històricament a la nissaga i pastisseria Climens del carrer Major, el 1996 l\'Ajuntament i la Colla van estrenar les rèpliques batejades com a Minga i Agneta en record de personatges populars del veïnat, preservant les peces originals al Saló de Plens com a joia patrimonial.',
      es: 'Los Cabezudos del Mercado son la pareja de figuras cómicas más antigua de Cardona y una de las más antiguas de toda Cataluña, con orígenes documentados que se remontan a la segunda mitad del siglo XVIII (1750-1800). Vinculados históricamente a la estirpe y pastelería Climens de la calle Mayor, en 1996 el Ayuntamiento y la Cuadrilla estrenaron las réplicas bautizadas como Minga y Agneta en recuerdo de personajes populares del barrio, preservando las piezas originales en el Salón de Plenos como joya patrimonial.',
      en: 'The Market Dwarfs are the oldest comic figures in Cardona and among the oldest in all Catalonia, with documented origins dating back to the late 18th century (1750–1800). Historically associated with the Climens bakery on Carrer Major, in 1996 the Town Council and troupe premiered the replicas named Minga and Agneta in memory of popular local townsfolk, safely preserving the original historic figures in Town Hall.'
    },
    vestimenta: {
      ca: 'En Minga llueix un barret de tres pics (tricorn) fosc amb rivet daurat, casaca vermella de gala amb botons rodons negres, corbatí blanc amb xulles de randa i llaç de la senyera catalana amb medalla. L\'Agneta porta mocador blanc de cap amb costura de ziga-zaga vermella, vestit blanc estampat de flors amb gran llaçada al coll, cinta de les quatre barres i el seu inconfusible pigall a la galta dreta.',
      es: 'Minga luce un sombrero de tres picos (tricornio) oscuro con ribete dorado, casaca roja de gala con botones redondos negros, corbatín blanco con encajes y lazo de la senyera catalana con medalla. Agneta lleva pañuelo blanco de cabeza con costura en zigzag roja, vestido blanco estampado de flores con gran lazada al cuello, cinta de las cuatro barras y su inconfundible lunar en la mejilla derecha.',
      en: 'Minga sports a dark tricorn hat with gold edging, red gala coat with round black buttons, white lace neckerchief, and medal with Catalan ribbon. Agneta wears a white headscarf with red zigzag stitching, floral-printed dress with oversized bow, four-striped ribbon, and her unmistakable beauty mark on her right cheek.'
    },
    ball: {
      ca: 'Ball de Nans de Cardona i Ballada del Barri del Mercat',
      es: 'Baile de Cabezudos de Cardona y Baile del Barrio del Mercado',
      en: 'Cardona Dwarfs Dance and Market Quarter Dance'
    },
    badge: {
      ca: 'Bicentenaris (S. XVIII)',
      es: 'Bicentenarios (S. XVIII)',
      en: 'Bicentenary Figures (18th C.)'
    },
    featured: true
  }
];
